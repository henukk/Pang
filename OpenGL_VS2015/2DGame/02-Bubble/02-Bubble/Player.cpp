...
void Player::init(const glm::ivec2 &tileMapPos, ShaderProgram &shaderProgram)
{
	...
	sprite->setNumberAnimations(5);  //ALERTA!!!!!    aixo s'ha de cnaviar quan afegim animacions
		...
		sprite->addKeyframe(MOVE_RIGHT, glm::vec2(0.1, 0.0f));
		sprite->addKeyframe(MOVE_RIGHT, glm::vec2(0.2, 0.0f));
		sprite->addKeyframe(MOVE_RIGHT, glm::vec2(0.3, 0.0f));
		sprite->addKeyframe(MOVE_RIGHT, glm::vec2(0.4, 0.0));

		sprite->setAnimationSpeed(DISPARA, 8);
		sprite->addKeyframe(DISPARA, glm::vec2(0.0f, 0.75f));
...
void Player::update(int deltaTime)
{
	...
	if (Game::instance().getKey(GLFW_KEY_0))
	{
		...
		//controlem el moviment de disparar
	...
	else
	{
		if(sprite->animation() == MOVE_LEFT)
			...
			sprite->changeAnimation(STAND_RIGHT);
		else if (sprite->animation() == DISPARA)
			sprite->changeAnimation(STAND_LEFT);
	}
