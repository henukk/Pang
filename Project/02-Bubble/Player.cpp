...
void Player::shootHarpoon() {
	...
	newHarpoon->init(glm::vec2(posPlayer.x + 16, posPlayer.y), texProgram); // Ajusta seg�n la posici�n del jugador
	newHarpoon->setTileMap(map);
	harpoons.push_back(newHarpoon); // A�ade el nuevo arp�n al vector
}
...
void Player::update(int deltaTime)
{
	...
	for (auto it = harpoons.begin(); it != harpoons.end(); ) {
		(*it)->update(deltaTime);
		if (!(*it)->isAlive()) {
			delete* it; // Elimina el objeto arp�n
